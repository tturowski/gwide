...
from gwidePlot_class import *
import argparse
from argparse import RawTextHelpFormatter
import select, os, yaml
...
output.add_argument("-o", "--output", dest="output", choices=["std", "aligner", "RTend", "ratio"], help="Select from following options:"+'\n\n'
                   "std - 5` and 3` end aligned only"+'\n'
                    "aligner - std plus chosen aligner (-l option)"+'\n'
                    "ratio - plot gwide ratio a exp / b exp"+'\n'
...
special.add_argument("-s", "--set_up", dest="set_up", type=float, help="Set up factor threshold (for -o gwide only)",
                  default=None)
...
if options.output == "ratio":
    ...
    if options.ntotal == True:
        data.ratio(to_divide=options.to_divide, divisor=options.divisor, exp_to_use='_ntotal')
        if options.select:
            data.ratio(to_divide=options.to_divide, divisor=options.divisor, exp_to_use='_ntotal', select=options.select)
    if options.nmax == True:
        data.ratio(to_divide=options.to_divide, divisor=options.divisor, exp_to_use='_nmax')
    data.ratio(to_divide=options.to_divide, divisor=options.divisor)
